...
uniform vec2 iMouse;
uniform vec3 phaseOffset;
uniform float lowerPhaseHue;
uniform float upperPhaseHue;
...
// Standard hue to RGB conversion function.
...
vec4 aurora(vec3 ro, vec3 rd)
{
    ...
    for(float i=0.;i<50.;i++)
    {
		...
		// col2.rgb = (sin(1. - phaseOffset + i * 0.043) * 0.5 + 0.5) * rzt;

		// Single hue
		// col2.rgb = hue2rgb(phaseHue) * rzt;

		// 2 hues blended
		float hueA = lowerPhaseHue; // seems to be more dominant since it is the lower one
		float hueB = upperPhaseHue;

        const int NUM_STEPS = 50;
        float u = float(i) / float(NUM_STEPS);      // goes 0…1 over your loop
        if (u < 0.5) {
            // first 50% of samples
            col2.rgb = hue2rgb(hueA) * rzt;
        } else {
            // last 50% of samples
            col2.rgb = hue2rgb(hueB) * rzt;
        }

		// // Varying around 1 hue
		// float baseHue = phaseHue;     // center hue (greenish-blue)
		// float hueRange = 0.2;    // how far we swing around baseHue
		// float hue = baseHue + hueRange * sin(i * 0.043);
		// // Then convert that hue to RGB and multiply by rzt
		// col2.rgb = hue2rgb(hue) * rzt;
...
void main() {
    ...
    else //Reflections
    {
		...
		float nz2 = triNoise2d((pos.xz+vec2(iTime/10.,iTime/12.))*vec2(.5,.7), 1.); // for animated water
